...
from constants import BOT_USERNAME, BOT_TOKEN
...
def process_update(update):
    ...
    Args:
        update (dict): The update data received from Telegram.
    """
    try:
        if "inline_query" in update:
            ...
            logger.warning("Received unhandled update: %s", update)
...
def process_inline_query(inline_query):
    ...
    Args:
        inline_query (dict): The inline query data.
    """
    ...
    try:
        ...
        profile_photo_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{profile_photo}" if profile_photo else ""
...
def process_callback_query(callback):
    ...
    Args:
        callback (dict): The callback query data.
    """
